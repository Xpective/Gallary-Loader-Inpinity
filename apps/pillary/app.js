...
const RENDER_MARGIN_ROWS = isMobile ? 3 : 6;
...
const RequestQueue = (()=> {
  ...
  function schedule(key, fn){
    ...
    const p = new Promise((resolve,reject)=>{ q.push({ fn, resolve, reject }); runNext(); });
    inflight.set(key, p);
...
function makeImg(idx){
  ...
  img.onerror = (e)=> { const el = e.currentTarget?.parentElement; if (el) el.classList.add("failed"); };
  return img;
}
function makeVideo(idx, posterUrl){
  ...
  v.onerror = (e)=> { const el = e.currentTarget?.parentElement; if (el) el.classList.add("failed"); };
  return v;
}
function videoTier(){
...
/* ======= Harter Global-Switch (Videos wirklich aus/an) ======= */
  ...
  try{
    ...
    data.forEach(meta=>{
      ...
      if (score != null) {
        ...
        if (toggleRarity?.checked) el.setAttribute("data-heat","1");
      }
    });
  }catch{}
}
...
if (toggleRarity) {
  toggleRarity.addEventListener("change", ()=>{
    ...
    // data-heat wird in loadMetaBatch gesetzt – hier nur CSS-Switch
  });
}
